...
internal class LlamaConfiguration(
  ...
  fun hasKv(layer: Int): Boolean = layer < nLayerKvFromStart
  fun headSize(layer: Int): Int = if (isSWA[layer]) headSizeSWA else headSizeFull
  fun numberOfKeyValueHeads(layer: Int): Int = numberOfKeyValueHeadsPerLayer[layer]
  fun kvDim(layer: Int): Int = numberOfKeyValueHeadsPerLayer[layer] * headSize(layer)
  fun queryDim(layer: Int): Int = numberOfHeads * headSize(layer)
  fun maxHiddenDim(): Int = feedForwardLength.max()
