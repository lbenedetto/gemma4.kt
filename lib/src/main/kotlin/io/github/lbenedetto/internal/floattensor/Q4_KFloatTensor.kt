...
internal class Q4_KFloatTensor(
  ...
  override fun get(index: Long): Float {
    ...
    val qsByte: Byte = readByte(memorySegment, qsOffset + group * 32 + nibbleIndex)
  ...
  companion object {
    ...
    fun getScaleMinK4(j: Int, mem: MemorySegment, scalesOffset: Long, isMin: Boolean): Int {
      if (j < 4) {
        ...
        return readByte(mem, scalesOffset + idx).toUnsignedInt() and 63
      } else {
        val lowIdx = j + 4
        val highIdx = if (isMin) j else j - 4
        val low = if (isMin)
          (readByte(mem, scalesOffset + lowIdx).toUnsignedInt() shr 4)
        else
          (readByte(mem, scalesOffset + lowIdx).toUnsignedInt() and 0xF)
        val high = (readByte(mem, scalesOffset + highIdx).toUnsignedInt() shr 6) and 0x3
          ...
          for (c in 0..1) {
            ...
            when (F_SPECIES.vectorBitSize()) {
              512 -> {
                ...
                value = loQ.fma(d1Vec, negM1Vec).fma(that.getFloatVector(F_SPECIES, loIdx), value)
              ...
              256 -> {
                ...
                value =
                  loQ0.fma(d1Vec, negM1Vec).fma(that.getFloatVector(F_SPECIES, loIdx), value)
                ...
                value =
                  hiQ0.fma(d2Vec, negM2Vec).fma(that.getFloatVector(F_SPECIES, hiIdx), value)
              ...
              128 -> {
                for (p in 0..3) {
                  ...
                  value = loQ.fma(d1Vec, negM1Vec).fma(
                    that.getFloatVector(
                      F_SPECIES,
