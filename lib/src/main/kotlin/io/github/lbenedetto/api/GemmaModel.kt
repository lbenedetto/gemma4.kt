...
class GemmaModel private constructor(private val model: Llama) {
  ...
  private fun doGenerate(
    ...
    val responseTokens = Llama.generateTokens(
      model, state, 0, promptTokens, stopTokens, config.maxTokens, sampler, echo = false, color = false, callback
    )
  ...
  companion object {
    ...
    fun load(modelPath: Path, contextLength: Int = DEFAULT_MAX_TOKENS): GemmaModel {
      val llama = AOT.tryUsePreLoaded(modelPath, contextLength) ?: ModelLoader.loadModel(modelPath, contextLength)
