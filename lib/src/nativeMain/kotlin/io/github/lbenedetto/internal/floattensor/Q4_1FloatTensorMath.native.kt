...
import ggml.bridge.ggml_bridge_dot_q4_1_f32_dequant
...
actual object Q4_1FloatTensorMath {
  internal actual fun vectorDot(
    ...
    var result = 0f
    var j = 0
    val blockSize = GGMLType.Q4_1.blockSize
    ...
    if (fullBlocks > 0) {
      val byteOffset = (thisOffset + j).toLong() / blockSize * GGMLType.Q4_1.typeSize
      val quantizedPtr = thiz.memorySegment.rawPointer(byteOffset)
      that.values.usePinned { pinned ->
        result += ggml_bridge_dot_q4_1_f32_dequant(fullBlocks, quantizedPtr, pinned.addressOf(thatOffset + j))
      }
      j += fullBlocks
    }
