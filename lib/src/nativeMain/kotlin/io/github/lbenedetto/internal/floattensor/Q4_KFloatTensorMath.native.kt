...
import ggml.bridge.ggml_bridge_dot_q4_K_f32_dequant
...
actual object Q4_KFloatTensorMath {
  internal actual fun vectorDot(
    ...
    var result = 0f
    var j = 0
    val blockSize = GGMLType.Q4_K.blockSize
    ...
    if (fullBlocks > 0) {
      val byteOffset = (thisOffset + j).toLong() / blockSize * GGMLType.Q4_K.typeSize
      val quantizedPtr = thiz.memorySegment.rawPointer(byteOffset)
      that.values.usePinned { pinned ->
        result += ggml_bridge_dot_q4_K_f32_dequant(fullBlocks, quantizedPtr, pinned.addressOf(thatOffset + j))
      }
      j += fullBlocks
    }
